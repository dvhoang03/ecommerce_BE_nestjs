...
export class IsValidQuantityValidator implements ValidatorConstraintInterface {
    ...
    async validate(value: number | number[], validationArguments: ValidationArguments): Promise<boolean> {
        if (typeof value === 'number') {
            let { productId, quantity } = validationArguments.object as CreateCartItemDTO;
            const cartItem = await this.cartItemService.findCartItemByProduct(productId);
            if (cartItem) {
                quantity = quantity + cartItem.quantity;
            }
            const results = await this.productService.IsValidProductQuantity(productId, quantity);
            return results;
        }
        else {
            const cartItemIds = value as number[];
